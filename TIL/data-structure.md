...
```go
x:=make([]int, 10, 100) // make(type | pointer, len, cap) 
...
뿐 만 아니라, 슬라이스를 잘라서 가져오는 경우에도 내부구조 배열을 새로 생성해서 가리킨다.  
예를 들어, `x[2:5]` 를 가져 온다면 길이가 3이고, 용량이 4인 내부구조 배열을 만든다.
(이 때의 배열 포인터는 `x[2]` 가 된다.)
